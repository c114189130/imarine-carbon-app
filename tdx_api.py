...
class TDXAPI:
    ...
    def get_highway_traffic(self, highway_id="1"):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return self._calc_congestion(avg_speed, "TDX API")
        except Exception as e:
    ...
    def _calc_congestion(self, avg_speed, source):
        ...
        return {"level": level, "level_text": text, "avg_speed": round(avg_speed, 1), "delay_factor": delay_map[level], "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "source": source}
...
def get_road_congestion(use_api=True, app_id=None, app_key=None):
    if use_api and app_id and app_key:
        ...
        delay_map = {"low": 1.0, "medium": 1.6, "high": 2.5}
        text_map = {"low": "🟢 順暢", "medium": "🟡 車多", "high": "🔴 壅塞"}
        return {"level": level, "level_text": text_map[level], "avg_speed": speed_map[level], "delay_factor": delay_map[level], "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "source": "模擬資料 (時段模擬)"}